...
class ControllerApiOrder extends Controller {
			...
			if ($order_info) {
				...
				if (!$json) {
                    ...
                    if(in_array($order_status_id, $this->config->get('config_complete_status'))){
                        $reward_points = $this->db->query("SELECT * FROM `" . DB_PREFIX . "customer_reward` WHERE order_id = '" . (int)$order_id . "'");
                        if(count($reward_points->rows) == 0){
                            $query = $this->db->query("SELECT sum(reward) AS total FROM " . DB_PREFIX . "order_product WHERE order_id = '" . (int)$order_id ."'");
                            $sql = "INSERT INTO `" . DB_PREFIX . "customer_reward` ( `customer_id`, `order_id`, `description`, `points`, `date_added`)
                        VALUES ('".$order_data['customer_id']."', '" . (int)$order_id ."', 'Order #$order_id', '".$query->row['total']."', NOW())";
                            $this->db->query($sql);
                        }
                    }
		...
		if (!isset($this->session->data['api_id'])) {
			...
			if ($order_info) {
				if(in_array($this->request->post['order_status_id'], $this->config->get('config_complete_status'))){
                    $reward_points = $this->db->query("SELECT * FROM `" . DB_PREFIX . "customer_reward` WHERE order_id = '" . (int)$order_id . "'");
                    if(!$reward_points->rows){
                        $query = $this->db->query("SELECT sum(reward) AS total FROM " . DB_PREFIX . "order_product WHERE order_id = '" . (int)$order_id ."'");
                        $sql = "INSERT INTO `" . DB_PREFIX . "customer_reward` ( `customer_id`, `order_id`, `description`, `points`, `date_added`)
                        VALUES ('".$order_info['customer_id']."', '" . (int)$order_id ."', 'Order #$order_id', '".$query->row['total']."', NOW())";
                        $this->db->query($sql);
                    }
                }
	...
	public function callMetriloApi()
	{
		$this->load->model('account/order');
		$this->load->model('tool/metrilo');
		$orders = $this->model_account_order->getOrders(0, 25);
	
		$ordersIds = array();
		
		foreach($orders as $order)
		{
			array_push($ordersIds, $order['order_id']);
		}
		
		$this->model_tool_metrilo->syncAllOrders($ordersIds);
	}
}
